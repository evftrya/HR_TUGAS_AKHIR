...
@section('page-name')
    <div class="flex items-center justify-between px-1 pt-4 pb-3">
        <div>
            <span class="font-medium text-2xl">Tambah COE</span>
            <div class="text-sm text-gray-600">Masukkan data Center of Excellence</div>
        </div>
    </div>
@endsection
...
@section('content-base')
    <div class="flex flex-col gap-4 w-full max-w-2xl mx-auto">
        ...
        <div class="rounded-md border p-6 bg-white">
            <form action="{{ route('manage.coe.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Nama COE</label>
                    <input type="text" name="nama_coe" value="{{ old('nama_coe') }}" class="mt-1 block w-full rounded-md border-gray-300" required>
                </div>
