...
<div x-data="{ open: {{ $isExpanded ? 'true' : 'false' }} }" class="border mb-2 border-gray-200 h-auto rounded-md">
    ...
    <div x-ref="container" class="overflow-hidden transition-all duration-300 bg-gray-50"
        :style="open ? 'max-height: max-content' : 'max-height: 0px'">
        <ul class="space-y-1 py-2 list-none p-0 h-auto border-t border-gray-100">
            {{ $slot }}
        </ul>
    </div>
</div>
