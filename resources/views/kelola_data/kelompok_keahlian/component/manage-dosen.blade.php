...
<template id="tmpl-dosen">
    <div class="flex flex-col items-center text-center p-5 bg-gray-50/50 rounded-2xl border border-transparent hover:border-blue-100 transition-all hover:shadow-md">
        ...
        <a href="" class="btn-lepas-plotting mt-4 text-[10px] font-bold text-red-500 hover:text-red-700 transition-colors border border-red-100 px-3 py-1 rounded-full hover:bg-red-50">
            Lepas Plotting
        </a>
    </div>
</template>
...
<script>
    ...
    // Pastikan request menangkap data, jika null berikan string kosong
    const reqDosenId = "{{ request('dosen_id') }}".trim();
    const reqSubKkId = "{{ request('sub_kk_id') }}".trim();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const urlLepasPlotting = "{{ route('manage.kelompok-keahlian.dosen-with-kk.lepas-dosen', ':DosenHasKK_id') }}";
                ...
                matchedKks.forEach(kk => {
                    ...
                    kk.subs.forEach(sub => {
                        ...
                        filteredDosens.forEach(dosen => {
                            ...
                            const finalUrl = urlLepasPlotting.replace(':DosenHasKK_id', dosen.id_pemetaan);
                            dsnTmpl.querySelector('.btn-lepas-plotting').href = finalUrl;
        ...
        function initDropdowns() {
            const dosenRaw = @json($dosen);
            const dosenList = Object.values(dosenRaw);
            ...
            dosenList.sort((a, b) => {
                ...
                opt.value = d.id;
                opt.textContent = d.pegawai_aktif?.nama_lengkap || 'Tanpa Nama';

                // Jika match, set atribut selected
                if (reqDosenId && String(d.id).toLowerCase() === reqDosenId.toLowerCase()) {
                    opt.setAttribute('selected', 'selected');
                    document.getElementById('placeholder-dosen').removeAttribute('selected');
                }
            ...
            database.forEach(fak => {
                ...
                fak.kks.forEach(kk => {
                    kk.subs.forEach(sub => {
                        ...
                        // Jika match, set atribut selected
                        if (reqSubKkId && String(sub.id).toLowerCase() === reqSubKkId.toLowerCase()) {
                            opt.setAttribute('selected', 'selected');
                            document.getElementById('placeholder-subkk').removeAttribute('selected');
                        }
            ...
            // PAKSA VALUE: Sangat penting untuk UUID
            if (reqDosenId) selectDosen.value = reqDosenId;
            if (reqSubKkId) selectSubKK.value = reqSubKkId;
        }
        ...
        btnToggle.onclick = () => {
            ...
            mainContent.classList.toggle('lg:w-9/12', isPanelVisible);
