...
<div class="min-h-screen font-sans gap-5 antialiased">
    ...
    <div class="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div class="absolute -top-32 left-1/2 h-80 w-80 -translate-x-1/2 
                    rounded-full bg-indigo-500 blur-[120px] opacity-40"></div>

        <div class="absolute bottom-0 left-0 h-72 w-72 rounded-full 
                    bg-violet-600 blur-[120px] opacity-30"></div>

        <div class="absolute bottom-10 right-0 h-80 w-80 rounded-full 
                    bg-emerald-500 blur-[120px] opacity-30"></div>
    </div>
    ...
    <main class="flex justify-center pb-5 border-b border-gray-200/70">
        <div class="w-full flex flex-wrap gap-6 justify-center">
            ...
            @for ($i = 0; $i < 2; $i++)
                <div class="bg-white max-w-64 rounded-3xl shadow-lg border border-gray-100
                    ...
                    <div class="flex flex-col items-center p-6 -mt-16">
                        <div class="w-28 h-28 rounded-full bg-gray-100 border-4 border-white shadow-md
                                    flex items-center justify-center">
                            <svg class="w-14 h-14 text-gray-500" xmlns="http://www.w3.org/2000/svg"
    ...
    <main id="top" class="mt-10 max-w-6xl mx-auto px-2">
        ...
        <section class="mb-8">
            <div class="flex flex-col items-end gap-3">
                ...
                <div class="flex flex-wrap gap-2">
                    <button class="history-filter-btn px-3 py-1.5 text-[11px] rounded-full 
        ...
        <section id="timeline">
            <div class="relative pl-8">
                ...
                <!-- Vertical Line -->
                <div class="absolute left-4 top-0 h-full w-[2px] bg-gradient-to-b 
                            from-indigo-500 via-slate-600 to-transparent"></div>

                <div class="space-y-7">

                    @forelse ($user['pengawakans'] as $pemetaan)
                        <article class="history-item relative pl-8 transition-all duration-200
                                        {{ $pemetaan->tmt_selesai ? 'opacity-70' : '' }}"
                                 data-category="{{ $pemetaan->formasi->bagian->type_work_position }}">

                            <!-- Marker -->
                            <div class="absolute left-0 top-2 w-3 h-3 rounded-full 
                                        bg-indigo-500 border-[3px] border-white shadow"></div>

                            <!-- Date -->
                            <div class="inline-flex items-center gap-2 px-3 py-1 text-white text-xs
                            ...
                            @if (session('account')['is_admin'])
                                <div class="flex items-center gap-2 mt-3">
                                    ...
                                    <button class="px-3 py-1 text-xs font-semibold bg-blue-100 
                    ...
                    @empty
                        <p class="text-gray-600 text-sm">Belum Ada Riwayat Jabatan.</p>
                    @endforelse
